  ...
  useEffect(() => {
    const fetchUserData = async () => {
      if (user?.uid) {
        ...
        if (docSnap.exists()) {
          setBudget(docSnap.data().monthlyBudgetCap || '');
        }
      }
    };
    fetchUserData();
      ...
      <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-3">
              {Object.entries(accentColors).map(([key, colors]) => (
                <button
                  ...
                  className={`w-10 h-10 rounded-full flex items-center justify-center transition-transform hover:scale-110 ${accentColor === key ? 'ring-2 ring-offset-2 ring-gray-400 dark:ring-gray-500 scale-110' : ''}`}
        ...
        <Card>
          ...
          <form onSubmit={handleSaveBudget}>
            ...
            {message && (
              <p className={`text-xs mt-3 ${message.includes('Failed') ? 'text-red-500' : 'text-green-600'} font-medium`}>
                {message}
              </p>
            )}
          </form>
